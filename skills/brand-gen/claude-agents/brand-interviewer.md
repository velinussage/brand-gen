...
- **source knowledge** (optional): from `brand_context_snapshot.source_knowledge`
...
Then call `brand_source_knowledge` when configured. Note metaphors, product truth, design principles already articulated, tensions, and what the brand is NOT. Keep brand scope strict; do not apply one brand's vault to another brand.
...
- **One question at a time** (use the `AskUserQuestion` tool, one question per invocation).
...
### Step 6: Apply elenchus when a belief doesn't hold
