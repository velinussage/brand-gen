...
import { defaultLearnings, saveLearnings, appendJournal, getRecentEntries } from '../src/memory.ts';
...
test('buildBrandGenContext combines saved brand + session state', async () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({ brand: { name: 'Acme Saved' }, messaging: { tagline: 'Reusable brand systems for modern teams' } }),
  );
  writeFileSync(
    join(workspaceDir, 'brand-identity.json'),
    JSON.stringify({ brand: { name: 'Acme Session' }, messaging: { tagline: 'Session-specific brand systems' } }),
  );
  writeFileSync(
  ...
  const bridge = new FakeBridge({
    ...
    brand_workspace_status: { workspaceKind: 'session', root },
  ...
  const context = await pluginTest.buildBrandGenContext(bridge as any, {
    ...
    logoIterateMcpPath: '/tmp/logo_iterate_mcp.py',
...
test('plugin register writes runtime status marker under canonical root', () => {
  const root = mkdtempSync(join(tmpdir(), 'brand-plugin-'));
  mkdirSync(join(root, 'brands', 'acme'), { recursive: true });
  ...
  const markerPath = join(root, 'runtime-status', 'plugins', 'openclaw-brand-gen.json');
  assert.equal(existsSync(markerPath), true);
  const marker = JSON.parse(readFileSync(markerPath, 'utf8'));
  assert.equal(marker.brandGenDir, root);
  assert.equal(marker.pluginName, 'openclaw-brand-gen');
});
...
test('runHeartbeatCycle performs discover + generate with deterministic policy', async () => {
  ...
  const bridge = new FakeBridge({
    ...
    brand_pipeline: {
      stopped_at: 'complete',
      result: { version_id: 'v7', image_paths: ['/tmp/v7.png'], agent_review_path: '/tmp/v7-agent-review.json', visual_review_status: 'pending', vlm_critique: { approved: true } },
    },
