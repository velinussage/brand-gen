...
Any agent with shell access can still fall back to raw `bgen` commands (the 100+ command CLI surface remains available). Example:
...
This works with older agent setups or when the MCP server can't be started. However, the preferred path for any new integration is the typed tool surface above — agents are more reliable when they navigate capabilities instead of scripting shell sequences. See [runtime-agent-contract.md](architecture/runtime-agent-contract.md) for the architecture invariants behind this split.
