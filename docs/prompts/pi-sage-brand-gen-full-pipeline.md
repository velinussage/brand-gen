...
3. Planning: use `brand_orchestrate_material` for the normal path, or `brand_prepare_run`, `brand_plan_run`, and `brand_validate_run` for stage-level fall-through. If the user names a platform/surface, map it to the closest supported material type (for example X/social → x/social card). If no surface or material type is implied, ask one concise question rather than inventing one. For exact visible copy, require deterministic text rendering (HTML/SVG/composite/text overlay strategy). Do not spend generation tokens on plans that fail exact-text, brand-fit, reference-readiness, or WCAG gates.
