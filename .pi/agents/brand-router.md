...
- Prefer `motion_specialist` for motion or animation work. `landing-hero` is a motion material now: a 16:9 hero background/sidecar animation with external page copy, not a static full hero-section mockup.
...
- If the user wants static art next to one-line hero copy, prefer `website-hero-illustration`. If they want animated landing-page media, use `landing-hero` and route to `motion_specialist`.
