...
def upload_file_to_replicate(token, file_path):
    """Upload a local file to Replicate's file hosting and return a serving URL."""
    p = Path(file_path)
    mime_map = {
        ".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".png": "image/png",
        ".gif": "image/gif", ".webp": "image/webp", ".bmp": "image/bmp",
    ...
    return result.get("urls", {}).get("get") or result.get("url") or ""


def _contains_data_uri(value):
    """Return True if a value contains any inline data: URI."""
...
def _replace_data_uri_value(token, value):
    """Recursively replace inline data: URIs with uploaded file URLs."""
    import tempfile
    ...
    if isinstance(value, str):
        ...
        with tempfile.NamedTemporaryFile(suffix=ext, delete=False) as f:
            ...
            tmp_path = f.name
        try:
            uploaded_url = upload_file_to_replicate(token, tmp_path)
            if uploaded_url:
                print(
                    f"  Uploaded inline asset ({len(raw)//1024}KB) to Replicate file hosting.",
                    file=sys.stderr,
                )
                return uploaded_url
            return value
    ...
    if isinstance(value, list):
        return [_replace_data_uri_value(token, item) for item in value]
    if isinstance(value, dict):
        return {key: _replace_data_uri_value(token, item) for key, item in value.items()}
    return value


def _replace_data_uris_with_uploads(token, input_data):
    """Replace data: URI values in input_data with uploaded file URLs."""
    return {
        key: _replace_data_uri_value(token, val)
...
def create_prediction(token, model_id, input_data, wait=True, _uploaded=False):
    ...
    try:
        ...
        if not _uploaded and _contains_data_uri(input_data):
            # Large payloads can cause connection drops — upload files and retry
            print("WARNING: Connection dropped with inline data; uploading files separately.", file=sys.stderr)
            uploaded_input = _replace_data_uris_with_uploads(token, input_data)
            return create_prediction(token, model_id, uploaded_input, wait=wait, _uploaded=True)
        if wait:
            # Prefer: wait causes long-lived connections that may drop — retry in async mode
            print("WARNING: Connection dropped; retrying without Prefer: wait.", file=sys.stderr)
            return create_prediction(token, model_id, input_data, wait=False, _uploaded=_uploaded)
        print("ERROR: Connection dropped by Replicate API.", file=sys.stderr)
        sys.exit(1)
...
def build_input(model_config, args, mode):
    ...
    # Style parameter (model-specific)
    if getattr(args, "style", None):
        style_field = field_map.get("style")
        if style_field:
            input_data[style_field] = args.style
        else:
            print(
                f"WARNING: Model '{args.model}' does not support --style parameter.",
                file=sys.stderr,
            )
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  %(prog)s image -b photo.jpg -p 'Add a copper gradient bar across the bottom with the episode title in white'\n"
    ...
    parser.add_argument("--style", "-s",
                        help="Model-specific style parameter (e.g., realistic_image, digital_illustration, vector_illustration, icon). Supported by recraft models.")
