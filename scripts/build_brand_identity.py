...
def build_material_prompt_snippets(profile: dict, design_language: dict, design_memory: dict) -> dict[str, dict[str, str]]:
    ...
    return {
        ...
        'concept_illustration': {
            'default': (
                f"For concept illustrations, create an abstract branded illustration that visualizes an idea, value, or concept — NOT a product screenshot. "
                f"Use {palette} as the dominant color language, {typography} for any text elements, and brand motifs derived from {mark_anatomy}. "
                f"Think editorial magazine illustration: confident, expressive, and metaphorical. "
                f"No fake UI, no dashboard chrome, no browser frames. Pure illustration with brand character. "
                f"Draw inspiration from illustration styles like unDraw, DrawKit, and Humaaans — clean, modern, and conceptual."
            ),
            'reference': (
                f"In reference mode, keep the illustration grounded in {brand_name}'s palette and mark motifs while matching the compositional confidence of the reference."
            ),
            'inspiration': (
                f"In inspiration mode, borrow the illustration style, confidence, and metaphor-building from the reference, but keep the color language and brand devices owned by {brand_name}."
            ),
            'hybrid': (
                f"In hybrid mode, use {brand_name}'s palette and motifs as the illustration's visual language while borrowing compositional energy and metaphor style from references."
            ),
        },
        'brand_scene': {
            'default': (
                f"For brand scenes, create atmospheric brand-world imagery — an environment, landscape, or abstract scene that establishes {brand_name}'s mood and feeling. "
                f"Use {palette} as the dominant color language. No UI elements, no text unless explicitly requested. "
                f"Think cinematic brand moment: evocative, immersive, and immediately recognizable as {brand_name}'s visual world. "
                f"Use approved devices such as {approved} as environmental motifs — woven into the scene, not pasted on top."
            ),
            'reference': (
                f"In reference mode, preserve {brand_name}'s palette and atmospheric feeling while borrowing the scene's spatial depth and environmental quality from the reference."
            ),
            'inspiration': (
                f"In inspiration mode, push atmosphere and emotional range further, but keep the scene recognizably within {brand_name}'s color world and brand feeling."
            ),
            'hybrid': (
                f"In hybrid mode, let {brand_name}'s palette and motifs define the scene's identity while references inform spatial confidence, lighting, and environmental richness."
            ),
        },
        'data_card': {
            'default': (
                f"For data cards, design a single-stat highlight card. One big number or metric dominates the upper half in the display font ({typography}). "
                f"Below: one sentence of context in the body font. Brand mark bottom-left. Solid brand color field from {palette} or subtle gradient. "
                f"The number should be the hero — large enough to read at thumbnail size. No charts, no graphs, just one bold stat."
            ),
            'reference': (
                f"In reference mode, preserve {brand_name}'s typography and palette while matching the stat-card clarity of the reference."
            ),
            'inspiration': (
                f"In inspiration mode, borrow the typographic drama and number presentation from the reference, but keep the card inside {brand_name}'s palette and font system."
            ),
            'hybrid': (
                f"In hybrid mode, use {brand_name}'s actual fonts and colors while borrowing stat-presentation confidence from references."
            ),
        },
        'quote_card': {
            'default': (
                f"For quote cards, place a customer or team quote in large italic or display font ({typography}) on a branded background from {palette}. "
                f"Attribution (name + role) below the quote in smaller body font. Brand mark anchored bottom-left. "
                f"Optional: large decorative quotation marks in a muted brand accent. Clean, editorial, minimal."
            ),
            'reference': (
                f"In reference mode, preserve {brand_name}'s palette and typography while matching the editorial quality of the reference."
            ),
            'inspiration': (
                f"In inspiration mode, borrow the quotation styling and editorial confidence from the reference, but keep the card recognizably {brand_name}."
            ),
            'hybrid': (
                f"In hybrid mode, use {brand_name}'s actual fonts, colors, and mark while borrowing editorial quote presentation from references."
            ),
        },
        'announcement_card': {
            'default': (
                f"For announcement cards, lead with a bold headline announcing a launch, feature, or milestone. "
                f"Use {typography} for the headline, {palette} for the background field. "
                f"Optional: one branded illustration or icon derived from {mark_anatomy} (not a screenshot) supporting the headline. "
                f"Brand mark + optional CTA at bottom. High energy but brand-disciplined."
            ),
            'reference': (
                f"In reference mode, preserve {brand_name}'s identity system while matching the announcement energy of the reference."
            ),
            'inspiration': (
                f"In inspiration mode, push headline drama and celebration energy from the reference, but keep the card inside {brand_name}'s actual visual system."
            ),
            'hybrid': (
                f"In hybrid mode, use {brand_name}'s palette, typography, and mark as the announcement's anchor while borrowing launch energy from references."
            ),
        },
        'process_card': {
            'default': (
                f"For process cards, show 3-5 numbered steps explaining how something works. "
                f"Each step gets a circled number in a brand accent from {palette} + a short label (5-8 words max) in {typography}. "
                f"Clean vertical flow with generous whitespace between steps. Optional small branded icons per step derived from {mark_anatomy}. "
                f"Brand mark bottom-left. The card should feel like a clear, scannable instruction set."
            ),
            'reference': (
                f"In reference mode, preserve {brand_name}'s typography and palette while matching the clarity and step presentation of the reference."
            ),
            'inspiration': (
                f"In inspiration mode, borrow the process visualization style from the reference, but keep the card recognizably inside {brand_name}'s visual system."
            ),
            'hybrid': (
                f"In hybrid mode, use {brand_name}'s actual design tokens while borrowing process-card layout and icon treatment from references."
            ),
        },
        'sticker_family': {
            'default': (
...
def build_identity(profile: dict) -> dict:
    ...
    return {
        ...
        'material_set_templates': {
            ...
            'social_content': {
                'description': 'Content-marketing social set with diverse card types for LinkedIn, Instagram, and X posts.',
                'materials': ['social', 'data-card', 'quote-card', 'announcement-card', 'content-card', 'editorial-card'],
            },
            'brand_storytelling': {
                'description': 'Illustration-first set for brand narrative — concept illustrations, atmospheric scenes, and process explainers.',
                'materials': ['concept-illustration', 'brand-scene', 'process-card', 'feature-illustration', 'campaign-poster'],
            },
            'podcast_launch': {
                'description': 'Podcast launch set with cover art, episode banners, and supporting social cards.',
                'materials': ['podcast-cover', 'podcast-banner', 'social', 'announcement-card'],
            },
        },
        'material_dna': {
            ...
            'concept_illustration': 'Abstract branded illustration that visualizes a concept or value through visual metaphor — uses brand palette and motifs, NOT product UI.',
            'brand_scene': 'Atmospheric brand-world imagery that establishes mood and feeling — an environment or abstract scene in the brand palette, no UI elements.',
            'data_card': 'Single-stat social card with one bold number + context sentence on a branded field — the metric is the hero.',
            'quote_card': 'Editorial testimonial card with a customer or team quote in display typography on a branded background.',
            'announcement_card': 'Launch or milestone announcement with bold headline and optional brand illustration — high energy, brand-disciplined.',
            'process_card': 'How-it-works card with numbered steps, clean vertical flow, and branded icons — scannable instruction set.',
