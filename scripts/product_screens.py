...
SHOT_PRESETS: dict[str, list[tuple[str, str]]] = {
    'sage-full': [
        ...
        ('governance', 'https://app.sageprotocol.io/governance'),
        ('communities', 'https://app.sageprotocol.io/communities'),
    ],
}
...
def parse_shots(args) -> list[tuple[str, str]]:
    ...
    if preset:
        if preset not in SHOT_PRESETS:
            raise SystemExit(f"Unknown preset '{preset}'. Available: {', '.join(sorted(SHOT_PRESETS))}")
        shots.extend(SHOT_PRESETS[preset])
    if not shots:
...
def cmd_capture(args) -> int:
    ...
    # Build the base command prefix.  When --cdp is supplied we prepend the
    # CDP flag so agent-browser connects to the user's real Chrome instance.
    cdp_port = getattr(args, 'cdp', None)
    base_cmd: list[str] = [agent_browser]
    if cdp_port:
        base_cmd += ['--cdp', str(cdp_port)]

    for label, url in shots:
        session = args.session or f'brand-product-{slugify(label)}-{time.time_ns()}-{os.getpid()}'
        env['AGENT_BROWSER_SESSION'] = session
        run([*base_cmd, 'open', url], env)
        run([*base_cmd, 'wait', '--load', 'networkidle'], env)
        for idx in range(1, args.count + 1):
            output = out_dir / f'{label}-{idx:02d}.png'
            run([*base_cmd, 'screenshot', '--full', str(output)], env)
            if idx != args.count:
                run([*base_cmd, 'scroll', 'down', str(args.scroll_px)], env)
                run([*base_cmd, 'wait', '--load', 'networkidle'], env)
        snapshot = out_dir / f'{label}-snapshot.txt'
        with snapshot.open('w') as fh:
            subprocess.run([*base_cmd, 'snapshot', '-i', '-c'], env=env, check=False, stdout=fh)
...
def main() -> int:
    ...
    capture.add_argument('--cdp', type=int, metavar='PORT', help='Connect to an already-running Chrome via CDP (e.g. 9222)')
