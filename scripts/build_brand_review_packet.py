...
def render_packet(version: str, entry: dict, brand_dir: Path, profile: dict) -> str:
    ...
    lines += [
        ...
        "[1-5, based on likely user preference. Mark favorite or rejected only if the user intent is already clear.]",
        "",
        "### Suggested feedback command",
        "python3 mcp/brand_iterate.py feedback vN --score <1-5> --notes \"<short reason>\" [--status favorite|rejected]",
        "",
        "### Next refinement",
        ...
        "",
        "## Scoring rule",
        "- The agent may suggest a score from the review, but the canonical saved score should reflect explicit or strongly implied user preference.",
