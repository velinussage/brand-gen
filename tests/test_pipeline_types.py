...
class PipelineTypesTests(unittest.TestCase):
    ...
    def test_plan_draft_converter_preserves_role_pack(self):
        typed = plan_draft_from_dict(
            {
                'plan': {
                    ...
                    'mode': 'hybrid',
                    'workflow_id': 'wf-plan',
                    'selected_reference_ids': ['ref_123'],
        ...
        self.assertEqual(typed.plan.selected_reference_ids, ['ref_123'])
        self.assertEqual(typed.plan.inspiration_board['direction_id'], 'direction_123')
    ...
    def test_critique_and_scratchpad_converters(self):
        ...
        scratchpad = scratchpad_from_dict(
            {
                ...
                'selected_reference_ids': ['ref_123'],
                'selected_inspiration_ids': ['source_456'],
                'selected_mechanic_ids': ['mechanic_789'],
                'inspiration_board': {'direction_id': 'direction_123'},
                'brand_dir': '/tmp/brand',
            },
            'wf-2',
        ...
        self.assertEqual(scratchpad.execution_prompt, 'short prompt')
        self.assertEqual(scratchpad.reference_analysis_mode, 'deterministic_only')
        self.assertEqual(scratchpad.reference_paths, ['/tmp/ref.png'])
        self.assertEqual(scratchpad.selected_reference_ids, ['ref_123'])
        self.assertEqual(scratchpad.selected_inspiration_ids, ['source_456'])
        self.assertEqual(scratchpad.selected_mechanic_ids, ['mechanic_789'])
        self.assertEqual(scratchpad.inspiration_board['direction_id'], 'direction_123')
