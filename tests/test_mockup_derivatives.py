...
from brand_gen.commands.generation import cmd_derive_mockup
...
class MockupDerivativeTests(unittest.TestCase):
    def test_cmd_derive_mockup_builds_payload_from_source_still(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("brand_gen.commands.generation.get_brand_dir", return_value=brand_dir), \
                 patch("brand_gen.commands.generation.load_manifest", return_value=manifest), \
                 patch("brand_gen.commands.generation.load_brand_memory", return_value=(brand_dir / "brand-profile.json", brand_dir / "brand-identity.json", {}, {})), \
                 patch("brand_gen.commands.generation.persist_generation_scratchpad_to_blackboard"), \
                 patch("brand_gen.commands.generation.append_run_event"), \
                 patch("brand_gen.commands.generation.execute_generation_scratchpad", return_value="v002") as execute_mock, \
